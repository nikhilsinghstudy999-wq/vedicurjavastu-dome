git add -f public/videos/Vedicvastuurja.mp4 && \
cat > "src/app/(marketing)/vishesh-upaye-1/page.tsx" << 'PAGEEOF'
...
PAGEEOF
git add . && git commit -m "Add video file directly and use local path" && git push